...
class WorkbenchMakeCommand extends Command
{
    ...
    protected function runCreator($package)
    {
        ...
        if (!array_key_exists('point_namespace_to_similar_dir', $config) || !is_bool($config['point_namespace_to_similar_dir'])) {
            $pointNsToSimilarDir = $this->confirm(PHP_EOL.++$this->step.'. Do you want PSR-4 autoloading standard for namespace '.$package->vendor.'\\'.$package->name.' in the composer.json file is pointed to the src/'.$package->vendor.'/'.$package->name.' directory (if say no, this will be pointed to the src directory) [yes|no] ?');
        } else {
        ...
        if ($this->option('resources')) {
            $this->question(PHP_EOL.++$this->step.'. Which of the following resources would you like to generate ?'.PHP_EOL);
    ...
    protected function buildPackage()
    {
        ...
        // Get author name
        $author_name = (empty($this->option('author-name'))) ? ((empty($config['name'])) ? $this->ask(PHP_EOL.++$this->step.'. What is your name ?') : $config['name']) : $this->option('author-name');

        // Get author email
        $author_email = (empty($this->option('author-email'))) ? ((empty($config['email'])) ? $this->ask(PHP_EOL.++$this->step.'. Hi '.$author_name.'! What is your e-mail address ?') : $config['email']) : $this->option('author-email');

        // Get description for package
        $description = $this->ask(PHP_EOL.++$this->step.'. You can provide a short descripton for your package here if you want:', '');
    ...
    protected function askForResources($resources)
    {
        ...
        foreach ($resources as $type => $path) {
            if ($this->confirm('   - ' .ucfirst(strtolower($type)). ' ?')) {
                $choices[] = $type;
            }
        }
